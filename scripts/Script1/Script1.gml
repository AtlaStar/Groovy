...
function GroovyTool() constructor {
	...
	ControlDock.add_tool(self)
}

function NullTool() : GroovyTool() constructor {}
...
function ControlDock() constructor {
	...
	static add_tool = function(struct) {
		array_push(singleton.tools, struct)
	}
	
	static tick = function(shape = undefined) {
		singleton.selected_tool = singleton.selected_tool ?? new NullTool()

		var result = singleton.selected_tool.action(shape ?? singleton.selected_shape)
		if !result.has_result  || is_instanceof(singleton.selected_tool, NullTool) {
			if mouse_check_button_pressed(mb_left) {
				var shape_under_mouse = singleton.current_canvas.hit_test()
				if singleton.selected_shape && singleton.selected_shape != shape_under_mouse
					singleton.selected_shape.set_wireframe_state(false, true)
				singleton.selected_shape = shape_under_mouse
			}
			var res = array_filter(singleton.tools, function(tool) {
				return keyboard_check_pressed(tool.shortcut_key)
			})
			
			if array_length(res) {
				//eventually add some priority system for tool shortcuts
				singleton.selected_tool = array_first(res)
			}
		} else if result.has_result && result.retick {
			singleton.tick(shape)	
		} else {
		}
	}
	
	
	static selected_tool = new SliceTool()
}

function PanTool() : GroovyTool() constructor {
	static icon = spr_pan_icon
	static shortcut_key = ord("P")
}

function SliceTool() : GroovyTool() constructor {
